...
get_league_general_info <- function(league_number) {
  ...
  everyones_points <- lapply(
    ...
    function(team_name) {
      df <- everyones_points |>
        dplyr::filter("entry"_name == team_name)
  ...
  general_information <- everyones_points |>
    dplyr::filter("event" == get_current_gw_number()) |>
    dplyr::select(
      "entry"_name, "points",
      "total_points", "overall_rank",
      "rank_change"
    ) |>
    dplyr::arrange(dplyr::desc("total_points"))
...
get_league_bench_points <- function(league_number) {
  ...
  most_points_on_bench <- everyones_points |>
    ...
    dplyr::transmute(bench = sum("points"_on_bench)) |>
    unique() |>
    dplyr::arrange(dplyr::desc("bench")) |>
    dplyr::ungroup()
...
get_league_team_value <- function(league_number) {
  ...
  highest_team_value <- everyones_points |>
    dplyr::filter("event" == get_current_gw_number()) |>
    dplyr::select("entry"_name, "value") |>
...
get_league_hits_taken <- function(league_number) {
  ...
  most_spent_on_transfers <- everyones_points |>
    dplyr::group_by("entry"_name) |>
    dplyr::transmute(transfer_cost = sum("event"_transfers_cost)) |>
    unique() |>
    dplyr::arrange(dplyr::desc("transfer_cost")) |>
    dplyr::ungroup()
